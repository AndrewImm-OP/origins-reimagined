...
public final class PresentationProfileManager extends MultiJsonDataLoader implements IdentifiableResourceReloadListener {
        ...
        loader.forEach((id, values) -> {
            for (JsonElement element : values) {
                try {
                    ...
                    OriginsReimagined.LOGGER.warn("Ignoring invalid Origins: Reimagined presentation profile {}: {}", id, exception.getMessage());
                }
            }
        });
        profiles = Map.copyOf(next);
        builtinOrigins = Set.copyOf(builtin);
        OriginsReimagined.LOGGER.debug("Loaded {} Origins: Reimagined presentation profiles", profiles.size());
        reloadCallback.run();
    }
    ...
    private static int parseColor(String value, Identifier resourceId) {
        try {
            ...
            OriginsReimagined.LOGGER.warn("Invalid accent color '{}' in profile {}", value, resourceId);
            return 0;
        }
    }
}
