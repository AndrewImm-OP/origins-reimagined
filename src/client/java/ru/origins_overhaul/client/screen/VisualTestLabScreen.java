...
import ru.origins_overhaul.client.visual.profile.PreviewState;
import ru.origins_overhaul.client.visual.profile.ResolvedVisualProfile;
import ru.origins_overhaul.client.visual.profile.VisualProfile;
...
import ru.origins_overhaul.client.visual.render.PreviewParticleSystem;
...
public final class VisualTestLabScreen extends Screen {
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.text(font, Component.literal("debug mode: " + OriginsOverhaul.DEBUG), 12, 23, 0xFF888888, false);
    ...
    private void exportReport() {
        String report = "Origins Overhaul Visual Report\nMinecraft=26.1.2\nprofile=" + (current() == null ? "none" : current().profileId()) + "\nmodel=" + modelOverride + "\nstate=" + stateName() + "\nactive=" + resolved().modifiers().stream().map(VisualModifier::id).toList() + "\nunsupported=" + resolved().unsupportedModifiers() + "\nconfig_particles=" + ClientSelectionConfig.visualParticles() + "\n";
        try { Files.writeString(Minecraft.getInstance().gameDirectory.toPath().resolve("logs/origins-overhaul-visual-report.txt"), report); Minecraft.getInstance().keyboardHandler.setClipboard(report); } catch (Exception exception) { OriginsOverhaul.LOGGER.warn("Unable to export visual report", exception); }
    }
