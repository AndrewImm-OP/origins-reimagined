    ...
    @Override protected void apply(Map<Identifier, List<JsonElement>> loader, ResourceManager manager, ProfilerFiller profiler) {
        ...
        loader.forEach((resourceId, values) -> values.forEach(json -> {
            try {
                ...
                OriginsOverhaul.LOGGER.warn("Ignoring invalid visual profile {}: {}", resourceId, exception.getMessage());
            }
        }));
        ...
        OriginsOverhaul.LOGGER.debug("Loaded {} Origins Overhaul visual profiles", profiles.size());
        reloadCallback.run();
    }
