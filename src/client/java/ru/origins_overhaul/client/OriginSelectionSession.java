...
public final class OriginSelectionSession {
    ...
    public List<OriginPresentation> currentOrigins() {
        if (!hasCurrentLayer() || ru.origins_overhaul.client.ClientOriginCatalog.getLayers().stream().noneMatch(l -> l.id().equals(currentLayerId()))) return List.of();
        var snapshot = ru.origins_overhaul.client.ClientOriginCatalog.getLayers().stream().filter(l -> l.id().equals(currentLayerId())).findFirst().orElse(null);
